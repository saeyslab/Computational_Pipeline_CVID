...
working_directory <- file.path(full_directory, "RF_noFS")
...
#set up writing
  ...
  for (experiment_day in c(13:33)){
    ...
    res3$prediction <- predict(res3, z_scored_test_data[,-c(1:3)])
  ...
  save(results_RF_2, results_RF_3, file = file.path(working_directory, paste0("Randomforests_NoFS_", to_use_data, ".Rdata")))

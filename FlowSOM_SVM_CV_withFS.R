...
working_directory <- file.path(full_directory, paste0("SVM_", kernel, "_WithFS"))
...
for (to_use_data in all){
  ...
  logFile = paste0(Logfile_directory,"/log_file_for_",
                   to_use_data,
                   "_FlowSOM_WithFS_SVM_", kernel, ".txt")
  ...
  for (experiment_day in c(13:33)){
    ...
    # Determine test and training data -- only perform wlicoxon on train data   
    ...
    # Now the there will be a seperation for the predicion of two and three classes
    ...
    z_scored_test_data <- suppressWarnings(UseZScoreTestData(test_data, 
                                                             ...
                                                             start = 2, 
                                                             stop = dimension-2))
    #Load in selected features
    load(paste0("FlowSOM/Tube", tube, "/Data", PID, "/Crossvalidation/RF_WithFS/Randomforests_WithFS_", to_use_data, ".Rdata"))
    
    selected_featurenames_2 <- selected_featureslist_2[[(experiment_day-12)]]
    selected_featurenames_3 <- selected_featureslist_3[[(experiment_day-12)]]
    ...
    res3 <- svm(diagnosis ~ ., data = selected_train_data_3[,-c(1,3)], kernel = "linear", cost =1, scale = TRUE)
    res3$prediction <- predict(res3, newdata = selected_test_data_3[,-c(1:3)])
    ...
    # For the two classes
    res2 <- svm(diagnosis_2 ~ ., data = selected_train_data_2[,-c(1,2)], kernel = "linear", cost =1, scale = TRUE)
  ...
  save(results_SVM_2, results_SVM_3, file = file.path(working_directory, paste0("SVM_", kernel, "_WithFS_", to_use_data, ".Rdata")))
